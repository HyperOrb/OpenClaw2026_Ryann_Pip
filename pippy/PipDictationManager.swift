//
//  PipDictationManager.swift
//  pippy
//
//  Shared push-to-talk dictation manager for the help chat and brainstorm Pip.
...
enum PipDictationPermissionProblem {
    case microphoneAccessDenied
    case speechRecognitionDenied
}

private enum PipDictationStartSource {
    case microphoneButton
    case keyboardShortcut
}

private struct PipDictationDraftCallbacks {
    let updateDraftText: (String) -> Void
    let submitDraftText: (String) -> Void
}

@MainActor
final class PipDictationManager: NSObject, ObservableObject {
    ...
    @Published private(set) var recordedAudioPowerHistory = Array(
        repeating: PipDictationManager.recordedAudioPowerHistoryBaselineLevel,
        count: PipDictationManager.recordedAudioPowerHistoryLength
    )
    ...
    @Published private(set) var currentPermissionProblem: PipDictationPermissionProblem?
    ...
    private let transcriptionProvider: any PipTranscriptionProvider
    private let audioEngine = AVAudioEngine()
    private var activeTranscriptionSession: (any PipStreamingTranscriptionSession)?
    private var activeStartSource: PipDictationStartSource?
    private var draftCallbacks: PipDictationDraftCallbacks?
    ...
    override init() {
        let transcriptionProvider = PipTranscriptionProviderFactory.makeDefaultProvider()
    ...
    private func startPushToTalk(
        startSource: PipDictationStartSource,
        currentDraftText: String,
        updateDraftText: @escaping (String) -> Void,
        submitDraftText: @escaping (String) -> Void,
        ...
        print("🎙️ PipDictationManager: start requested (\(startSource))")

        if needsInitialPermissionPrompt {
            print("🎙️ PipDictationManager: requesting initial permissions")
            NSApplication.shared.activate(ignoringOtherApps: true)
        ...
        guard await requestMicrophoneAndSpeechPermissionsWithoutDuplicatePrompts() else {
            print("🎙️ PipDictationManager: permissions missing or denied")
            isPreparingToRecord = false
            return
        }
        guard !Task.isCancelled else {
            print("🎙️ PipDictationManager: start cancelled (shortcut released during permission check)")
            isPreparingToRecord = false
            return
        }
        guard pendingStartRequestIdentifier == startRequestIdentifier else {
            print("🎙️ PipDictationManager: start request superseded")
            isPreparingToRecord = false
            return
        }
        ...
        draftCallbacks = PipDictationDraftCallbacks(
            updateDraftText: updateDraftText,
            submitDraftText: submitDraftText
        )
        ...
        guard !Task.isCancelled else {
            print("🎙️ PipDictationManager: start cancelled (shortcut released before recording began)")
            resetSessionState()
            return
        }
        ...
        do {
            ...
            guard !Task.isCancelled else {
                print("🎙️ PipDictationManager: start cancelled (shortcut released during session start)")
                audioEngine.stop()
                audioEngine.inputNode.removeTap(onBus: 0)
                activeTranscriptionSession?.cancel()
                resetSessionState()
            ...
            isPreparingToRecord = false
            print("🎙️ PipDictationManager: recognition session started")
        } catch {
            isPreparingToRecord = false
            ...
            if isCancelledByUser || isTransientSocketError {
                ...
                print("🎙️ PipDictationManager: start cancelled (\(reason))")
                resetSessionState()
                return
            }
            ...
            print("❌ PipDictationManager: failed to start recognition session (\(transcriptionProvider.displayName)): \(error)")
            resetSessionState()
        }
    }

    private func stopPushToTalk(expectedStartSource: PipDictationStartSource) {
        pendingStartRequestIdentifier = UUID()
        ...
        print("🎙️ PipDictationManager: stop requested (\(expectedStartSource))")
    ...
    private func startRecognitionSession() async throws {
        ...
        print("🎙️ PipDictationManager: opening transcription provider \(transcriptionProvider.displayName)")
        ...
        print("🎙️ PipDictationManager: provider ready, starting audio engine")
    ...
    private func handleRecognitionError(_ error: Error) {
        ...
        if isFinalizingTranscript && !latestRecognizedText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            ...
            print("❌ Pip dictation error (\(transcriptionProvider.displayName)): \(error)")
            lastErrorMessage = userFacingErrorMessage(
                from: error,
