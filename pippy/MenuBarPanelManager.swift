...
extension Notification.Name {
    static let pipDismissPanel = Notification.Name("pipDismissPanel")
}
...
final class MenuBarPanelManager: NSObject {
    ...
    private func createStatusItem() {
        ...
        button.image = makePipMenuBarIcon()
        button.image?.isTemplate = true
        button.action = #selector(statusItemClicked)
        button.target = self
    }

    /// Draws a simple template-safe puppy paw for Pip's menu bar icon.
    private func makePipMenuBarIcon() -> NSImage {
        let iconSize: CGFloat = 18
        let image = NSImage(size: NSSize(width: iconSize, height: iconSize))
        image.lockFocus()

        NSColor.black.setFill()
        NSBezierPath(ovalIn: NSRect(x: 5.2, y: 3.4, width: 7.6, height: 7.0)).fill()
        NSBezierPath(ovalIn: NSRect(x: 3.0, y: 10.0, width: 3.8, height: 4.8)).fill()
        NSBezierPath(ovalIn: NSRect(x: 7.1, y: 11.2, width: 3.8, height: 4.9)).fill()
        NSBezierPath(ovalIn: NSRect(x: 11.3, y: 10.0, width: 3.8, height: 4.8)).fill()
