...
class WindowPositionManager {
    ...
    private static let hasUserConfirmedFullComputerAccessUserDefaultsKey = "com.pip.hasUserConfirmedFullComputerAccess"
    ...
    /// Returns true when the app should proceed without blocking on an
    /// accessibility re-check. Useful for dev builds where macOS can briefly
    /// report a false negative after relaunch.
    static func shouldTreatAccessibilityPermissionAsGrantedForSessionLaunch() -> Bool {
        let hasAccessibilityPermissionNow = hasAccessibilityPermission()
        let hasPreviouslyConfirmedAccessibilityPermission = UserDefaults.standard.bool(
            forKey: hasPreviouslyConfirmedAccessibilityPermissionUserDefaultsKey
        )
        return hasAccessibilityPermissionNow
            || hasPreviouslyConfirmedAccessibilityPermission
            || hasUserConfirmedFullComputerAccess()
    }

    static func markFullComputerAccessConfirmedByUser() {
        UserDefaults.standard.set(true, forKey: hasUserConfirmedFullComputerAccessUserDefaultsKey)
        UserDefaults.standard.set(true, forKey: hasPreviouslyConfirmedAccessibilityPermissionUserDefaultsKey)
        UserDefaults.standard.set(true, forKey: hasPreviouslyConfirmedScreenRecordingPermissionUserDefaultsKey)
        UserDefaults.standard.set(true, forKey: "hasScreenContentPermission")
    }

    static func hasUserConfirmedFullComputerAccess() -> Bool {
        UserDefaults.standard.bool(forKey: hasUserConfirmedFullComputerAccessUserDefaultsKey)
    }
    ...
    static func requestAutomationPermissionsForCommonApps() {
        let automationTargets: [(displayName: String, bundlePath: String?, scriptSource: String)] = [
            (
                displayName: "Finder",
                bundlePath: "/System/Library/CoreServices/Finder.app",
                scriptSource: #"tell application "Finder" to get name"#
            ),
            (
                displayName: "Safari",
                bundlePath: "/Applications/Safari.app",
                scriptSource: #"tell application "Safari" to get name"#
            ),
            (
                displayName: "Google Chrome",
                bundlePath: "/Applications/Google Chrome.app",
                scriptSource: #"tell application "Google Chrome" to get name"#
            ),
            (
                displayName: "Notes",
                bundlePath: "/System/Applications/Notes.app",
                scriptSource: #"tell application "Notes" to get name"#
            ),
            (
                displayName: "System Events",
                bundlePath: "/System/Library/CoreServices/System Events.app",
                scriptSource: #"tell application "System Events" to get name"#
            )
        ]

        for (targetIndex, target) in automationTargets.enumerated() {
            if let bundlePath = target.bundlePath,
               !FileManager.default.fileExists(atPath: bundlePath) {
                continue
            }

            DispatchQueue.main.asyncAfter(deadline: .now() + Double(targetIndex) * 0.7) {
                var errorDictionary: NSDictionary?
                let script = NSAppleScript(source: target.scriptSource)
                _ = script?.executeAndReturnError(&errorDictionary)
                if let errorDictionary {
                    print("⚠️ Automation permission request for \(target.displayName): \(errorDictionary)")
                } else {
                    print("✅ Automation permission request reached \(target.displayName)")
                }
            }
        }
    }
