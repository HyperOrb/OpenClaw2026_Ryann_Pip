...
//  One OverlayWindow is created per screen so the cursor Pip
...
private struct PipPetIconView: View {
    var body: some View {
        ZStack {
            Circle()
...
/// The Pip's behavioral mode. Controls whether it follows the cursor,
/// is flying toward a detected UI element, or is pointing at an element.
enum PipNavigationMode {
    /// Default — Pip follows the mouse cursor with spring animation
    case followingCursor
    /// Pip is animating toward a detected UI element location
    case navigatingToTarget
    /// Pip has arrived at the target and is pointing at it with a speech bubble
    case pointingAtTarget
}

// SwiftUI view for the blue glowing cursor pointer.
// Each screen gets its own PipPetOverlayView. The view checks whether
// the cursor is currently on THIS screen and only shows the Pip
...
struct PipPetOverlayView: View {
    ...
    init(screenFrame: CGRect, isFirstAppearance: Bool, companionManager: CompanionManager) {
        ...
        // Pip doesn't flash at (0,0) before onAppear fires.
    ...
    // MARK: - Pip Navigation State

    /// The Pip's current behavioral mode (following cursor, navigating, or pointing).
    @State private var pipNavigationMode: PipNavigationMode = .followingCursor
    ...
    /// Scale factor applied to the Pip triangle during flight. Grows to ~1.3x
    /// at the midpoint of the arc and shrinks back to 1.0x on landing, creating
    /// an energetic "swooping" feel.
    @State private var pipFlightScale: CGFloat = 1.0

    /// Scale factor for the navigation speech bubble's pop-in entrance.
    /// Starts at 0.5 and springs to 1.0 when the first character appears.
    @State private var navigationBubbleScale: CGFloat = 1.0

    /// True when the Pip is flying BACK to the cursor after pointing.
    ...
    private let fullWelcomeMessage = "hey! i'm pip"
    ...
    var body: some View {
        ZStack {
            ...
            // Navigation pointer bubble — shown when Pip arrives at a detected element.
            // Pops in with a scale-bounce (0.5x → 1.0x spring) and a bright initial
            // glow that settles, creating a "materializing" effect.
            if pipNavigationMode == .pointingAtTarget && !navigationBubbleText.isEmpty {
            ...
            // Pip pet marker — shown when idle or while TTS is playing (responding).
            ...
            PipPetIconView()
                ...
                .shadow(color: DS.Colors.overlayCursorBlue, radius: 8 + (pipFlightScale - 1.0) * 20, x: 0, y: 0)
                .scaleEffect(pipFlightScale)
                .opacity(pipIsVisibleOnThisScreen && (companionManager.voiceState == .idle || companionManager.voiceState == .responding) ? cursorOpacity : 0)
                .position(cursorPosition)
                .animation(
                    pipNavigationMode == .followingCursor
                ...
                .animation(
                    pipNavigationMode == .navigatingToTarget ? nil : .easeInOut(duration: 0.3),
                    value: triangleRotationDegrees
                )

            // Pip waveform — replaces the triangle while listening
            PipPetWaveformView(audioPowerLevel: companionManager.currentAudioPowerLevel)
                .opacity(pipIsVisibleOnThisScreen && companionManager.voiceState == .listening ? cursorOpacity : 0)
                .position(cursorPosition)
                .animation(.spring(response: 0.2, dampingFraction: 0.6, blendDuration: 0), value: cursorPosition)
                .animation(.easeIn(duration: 0.15), value: companionManager.voiceState)

            // Pip spinner — shown while the AI is processing (transcription + Ollama + waiting for TTS)
            PipPetSpinnerView()
                .opacity(pipIsVisibleOnThisScreen && companionManager.voiceState == .processing ? cursorOpacity : 0)
                .position(cursorPosition)
                .animation(.spring(response: 0.2, dampingFraction: 0.6, blendDuration: 0), value: cursorPosition)
                .animation(.easeIn(duration: 0.15), value: companionManager.voiceState)
        ...
        .onChange(of: companionManager.detectedElementScreenLocation) { newLocation in
            // When a UI element location is detected, navigate the Pip to
    ...
    /// Whether the Pip triangle should be visible on this screen.
    ...
    /// screen isn't the one animating), hide the cursor so only one Pip
    /// is ever visible at a time.
    private var pipIsVisibleOnThisScreen: Bool {
        switch pipNavigationMode {
        case .followingCursor:
            // If another screen's PipPetOverlayView is navigating to an element,
            // hide the cursor on this screen to prevent a duplicate Pip
    ...
    private func startTrackingCursor() {
        timer = Timer.scheduledTimer(withTimeInterval: 0.016, repeats: true) { _ in
            ...
            // During forward flight or pointing, the Pip is NOT interrupted by
            // mouse movement — it completes its full animation and return flight.
            // Only during the RETURN flight do we allow cursor movement to cancel
            // (so the Pip snaps to following if the user moves while it's flying back).
            if self.pipNavigationMode == .navigatingToTarget && self.isReturningToCursor {
            ...
            if self.pipNavigationMode != .followingCursor {
                return
            }

            // Normal cursor following
            let swiftUIPosition = self.convertScreenPointToSwiftUICoordinates(mouseLocation)
            let pipX = swiftUIPosition.x + 35
            let pipY = swiftUIPosition.y + 25
            self.cursorPosition = CGPoint(x: pipX, y: pipY)
        }
    }
    ...
    /// Starts animating the Pip toward a detected UI element location.
    private func startNavigatingToElement(screenLocation: CGPoint) {
        ...
        // Offset the target so the Pip sits beside the element rather than
        ...
        pipNavigationMode = .navigatingToTarget
        isReturningToCursor = false

        animateBezierFlightArc(to: clampedTarget) {
            guard self.pipNavigationMode == .navigatingToTarget else { return }
            self.startPointingAtElement()
        }
    }

    /// Animates the Pip along a quadratic bezier arc from its current position
    ...
    private func animateBezierFlightArc(
        ...
        // upward (negative Y in SwiftUI) so the Pip flies in a parabolic arc.
        ...
        navigationAnimationTimer = Timer.scheduledTimer(withTimeInterval: frameInterval, repeats: true) { _ in
            ...
            // Pip grows to ~1.3x at the apex, then shrinks back to 1.0x on landing.
            let scalePulse = sin(linearProgress * .pi)
            self.pipFlightScale = 1.0 + scalePulse * 0.3
        }
    }
    ...
    private func startPointingAtElement() {
        pipNavigationMode = .pointingAtTarget
        ...
        streamNavigationBubbleCharacter(phrase: pointerPhrase, characterIndex: 0) {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) {
                guard self.pipNavigationMode == .pointingAtTarget else { return }
    ...
    private func streamNavigationBubbleCharacter(
        ...
        guard pipNavigationMode == .pointingAtTarget else { return }
    ...
    /// Flies the Pip back to the current cursor position after pointing is done.
    ...
    /// Returns the Pip to normal cursor-following mode after navigation completes.
...
private struct PipPetWaveformView: View {
...
private struct PipPetSpinnerView: View {
...
// Pip seamlessly follows the cursor across multiple monitors.
@MainActor
class OverlayWindowManager {
    ...
    func showOverlay(onScreens screens: [NSScreen], companionManager: CompanionManager) {
        ...
        for screen in screens {
            ...
            let contentView = PipPetOverlayView(
